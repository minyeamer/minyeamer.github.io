async function generatePreview(e){const t=e.getAttribute("data-url");if(!t)return;try{const n=await getLinkPreview(t,{headers:{"User-Agent":"Mozilla/5.0 (compatible; HugoLinkPreview/1.0)"}});console.log("Fetched data:",n);const s=`
    ...
    `;e.innerHTML=s,console.log("Preview generated for:",t)}catch(n){console.error("미리보기 생성 실패:",n),e.innerHTML=`<a href="${t}" target="_blank">${t}</a>`}}document.addEventListener("DOMContentLoaded",()=>{const e=document.querySelectorAll(".link-preview");e.forEach(generatePreview)})