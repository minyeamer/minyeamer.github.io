...
그 외 Hugo 설정은 아래 [Hugo 설정 가이드 문단](#hugo-설정-가이드)을 참고해주시기 바랍니다.
...
서택스 테마의 주요 기능을 활용하기 위해 Hugo 설정 파일에 추가해야 하는 파라미터들을 안내합니다.

제가 작성하고 사용하는 설정 파일을 링크로 첨부합니다.

{{< bookmark "https://github.com/minyeamer/minyeamer.github.io/blob/main/config.yaml" >}}

### 기본 설정

```yaml
theme: "seotax"
defaultContentLanguage: "ko"  # 기본 언어 설정

params:
  posts:
    section: "posts"  # 게시글이 위치한 디렉토리 경로
```

- `theme`: 테마 디렉토리명 지정
- `defaultContentLanguage`: 다국어 번역의 기본 언어 (i18n 기능과 연동)
- `params.posts.section`: 게시글이 저장된 content 하위 경로 (`content/posts/` 사용 시 "posts")
...
params:
  assets:
    ...
    safari_pinned_tab: "/images/favicons/apple-touch-icon.png"
    opengraph: "/images/profile/opengraph.jpg"  # SNS 공유 시 표시될 이미지
```

- 파비콘 및 소셜 미디어 공유 이미지 경로 지정
- `static/` 디렉토리 기준 상대 경로 사용
...
params:
  menu:
    profileImage: "/images/profile/menu.jpg"  # 메뉴 상단 프로필 이미지
    categories: true  # 카테고리 트리 표시 여부
    recentPosts: true  # 최근 게시글 목록 표시 여부
```

- `profileImage`: 사이드 메뉴 최상단에 표시할 프로필 이미지 경로
- `categories`: `true` 로 설정 시 카테고리 트리 표시
- `recentPosts`: `true` 로 설정 시 최신글 5개까지 표시
...
- 사이드 메뉴 하단에 소셜 링크 아이콘 표시
- `github`, `twitter` 외에도 확장 가능하지만, 현재는 `github` 만 지원
...
params:
  tableOfContents:
    startLevel: 2  # 목차에 표시할 제목 시작 레벨 (h2)
    endLevel: 3    # 목차에 표시할 제목 종료 레벨 (h3)
...
- `params.tableOfContents`: 우측 사이드바 목차 설정
- `markup.tableOfContents`: Hugo 마크다운 렌더링 시 목차 생성 범위
...
params:
  GoogleAnalytics:
    tagId: "G-XXXXXXXXXX"  # GA4 측정 ID

  searchEngine:
    google:
      siteVerificationTag: "verification-code"  # Google Search Console 인증 코드
    naver:
      siteVerificationTag: "verification-code"  # 네이버 서치어드바이저 인증 코드

  schema:
    publisherType: "Person"  # 또는 "Organization"
    sameAs:  # 작성자의 다른 웹사이트/SNS 링크
      - "https://example.com/"
```

- `GoogleAnalytics.tagId`: Google Analytics 4 추적 코드
- `searchEngine`: 검색 엔진 소유권 확인 메타 태그 삽입
- `schema`: Schema.org 구조화 데이터 (검색 엔진 최적화)

#### URL 구조 최적화

```yaml
permalinks:
  posts: "/blog/:slugorcontentbasename/"  # 게시글 URL 형식
```

- `:slugorcontentbasename`: 파일명 또는 front matter의 `slug` 값 사용
- SEO 친화적인 URL 구조 설정
...
```yaml
params:
  serviceWorker: "precache"  # "precache" 또는 "disabled"
```

- `"precache"`: 오프라인 캐싱 활성화 (PWA 지원)
- `"disabled"`: 서비스 워커 비활성화
...
params:
  comments:
    enabled: true
    disqusShortname: "your-disqus-shortname"  # Disqus 사이트명
```

- `comments.enabled`: 댓글 영역 활성화 여부
- `comments.disqusShortname`: Disqus에서 발급받은 사이트 shortname
...
params:
  search:
    enabled: true  # 검색 기능 활성화

disableKinds: ["term"]  # Taxonomy 정적 페이지 생성 비활성화
```

- `params.search.enabled`: 검색 모달 및 검색 페이지 활성화
- `disableKinds`: `["term"]`으로 설정 시 카테고리/태그별 정적 페이지 생성 중지 (동적 검색 페이지로 대체)

#### 다국어 번역

```yaml
params:
  i18nDir: "themes/seotax/i18n"  # 번역 파일 경로
```

- `params.i18nDir`: YAML 번역 파일이 위치한 디렉토리 (기본값: `themes/seotax/i18n`)
- 툴바 우측 상단의 언어 선택기와 연동되며, 27개 언어 지원
